...
public class MoonOrbitCalculator  {
		...
		public static OrbitalElements.OrbitalElementsPieces Calculate(double t){

            
	    var DEG_TO_RAD = SO.DEG_TO_RAD;
		...
		var sg = 0.0011776 * Math.Cos(DEG_TO_RAD * (49.1562 - 75869.8120 * t + 35.458 * t2e4 
			...
			var computed = new OrbitalElements.OrbitalElementsPieces(){
				a = sma * 1000,
				e = ecc,
				i = 2.0 * Math.Asin(gamma) * SO.RAD_TO_DEG,
				w = ( (lp - raan)) % 360,
				o = ( raan) % 360,
				M = ( (lambda - lp)) %  360
			};
